...
# GA4 measurement id (e.g. "G-XXXXXXX"). Empty by default so local/dev builds load NO
# analytics. Set MEASUREMENT_ID in the deploy environment to switch tracking on.
MEASUREMENT_ID = os.environ.get("MEASUREMENT_ID", "").strip()
...
STR = {
    "en": {
        ...
        "tool_cta_text": "All 112 foods live in one interactive table — search any food, sort by any amino acid or DIAAS, the shading shows high vs low, and you can compare up to 4 side by side.",
        "tool_cta_btn": "Open the interactive table →",
        "home_caps": "Search any food · sort by any amino acid or DIAAS · colour shows high vs low · compare up to 4 side by side",
    ...
    "zh": {
        ...
        "tool_cta_text": "全部 112 种食物都在一张交互表里——搜任意食物、按任一氨基酸或 DIAAS 排序、颜色显示高低、最多并排对比 4 种。",
        "tool_cta_btn": "打开交互表格 →",
        "home_caps": "搜任意食物 · 按任一氨基酸或 DIAAS 排序 · 颜色显示高低 · 最多并排对比 4 种",
        "abbr_title": "缩写说明",
...
HUBS = [
    {"slug": "protein-quality-ranking",
     "title": {"en": "Best protein sources, ranked by quality (DIAAS)",
               "zh": "最佳蛋白质来源排行：按 DIAAS 蛋白质量排序"},
     "intro": {"en": "Every food here with a published DIAAS, ranked from the best protein sources down. FAO tiers: DIAAS ≥100 is excellent, 75–99 high quality, below 75 carries no quality claim.",
               "zh": "本站所有有已发表 DIAAS 的食物，从最优蛋白质来源往下排。FAO 三档：DIAAS ≥100 为优质，75–99 为高质量，低于 75 不作质量声明。"},
    ...
    {"slug": "best-plant-protein",
     "title": {"en": "Best plant protein: complete plant proteins ranked by DIAAS",
               "zh": "最佳植物蛋白：完整植物蛋白按 DIAAS 排行"},
...
def build():
    ...
    base_ctx = {"SITE_URL": SITE_URL, "LANGS": LANGS, "HTML_LANG": HTML_LANG,
                "DISCLAIMER": DISCLAIMER, "MEASUREMENT_ID": MEASUREMENT_ID}
    ...
    print(f"MEASUREMENT_ID = {MEASUREMENT_ID or '(unset — analytics off)'}")
    validate_pages()
...
# Pages that legitimately carry none of the SEO/compliance tags below.
# 404 is noindex; we also skip any page that opts out via robots noindex.
...
def validate_pages():
    ...
    description, and a disclaimer. noindex pages (e.g. 404) are exempt."""
    problems = []
    ...
    if problems:
        print("\nBUILD CHECK FAILED — indexable pages missing required elements:")
        for rel, miss in problems:
            print(f"  {rel}: {', '.join(miss)}")
        raise SystemExit(1)
    print(f"Build check passed: {checked} indexable pages all carry "
          f"canonical + hreflang + meta description + disclaimer "
          f"({len(pages) - checked} noindex page(s) exempt).")


def write_measurement():
    ga_state = (f"configured (`{MEASUREMENT_ID}`)" if MEASUREMENT_ID
                else "not set — analytics is OFF until `MEASUREMENT_ID` is exported at build time")
    doc = f"""# Measurement — Protein Map / 蛋白质地图
...
|----|--------|------------------|
| M1 | Indexed-page count | Search Console → Pages (Indexed) vs the ~{1 + len(LANGS) * (1 + 4 + 1 + len(HUBS) + 112)} URLs built |
...
## Analytics setup
GA4 status for this build: **{ga_state}**.
...
5. Submit the sitemap: `{SITE_URL}/sitemap.xml` under **Sitemaps**.
...
"""
    (OUT / "MEASUREMENT.md").write_text(doc, encoding="utf-8")
